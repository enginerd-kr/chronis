...
def _insert_misfired_job(
    storage, job_id, policy, trigger_type="interval", trigger_args=None, minutes_late=5
):
...
class TestMisfireRunOnceExecution:
    ...
    def test_run_once_misfired_job_executes_and_reschedules(self):
        ...
        _insert_misfired_job(storage, "misfired-1", policy="run_once", trigger_args={"minutes": 5})
        ...
        scheduler._execution_coordinator.try_execute(job_data, scheduler._job_queue.mark_completed)
        time.sleep(0.5)
...
class TestMisfireSkipExecution:
    ...
    def test_skip_misfired_job_not_executed_but_rescheduled(self):
        ...
        _insert_misfired_job(storage, "skip-1", policy="skip", trigger_args={"minutes": 5})
...
class TestMisfireRunAllExecution:
    ...
    def test_run_all_catches_up_incrementally(self):
        ...
        _insert_misfired_job(
            storage, "runall-1", policy="run_all", trigger_args={"minutes": 5}, minutes_late=15
        )
        ...
        scheduler._execution_coordinator.try_execute(job_data2, scheduler._job_queue.mark_completed)
        time.sleep(0.5)
