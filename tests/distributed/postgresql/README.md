# Distributed Scheduler Test (PostgreSQL + Redis)

This example simulates a Kubernetes environment using Docker Compose with PostgreSQL for storage and Redis for distributed locking.

## Architecture

- **PostgreSQL**: Shared storage for job data
...
```bash
cd tests/distributed/postgresql
...
- **Hybrid storage approach**: PostgreSQL for persistent storage + Redis for fast locking

This simulates a real Kubernetes deployment with multiple scheduler pods sharing PostgreSQL for data persistence and Redis for distributed lock coordination.
